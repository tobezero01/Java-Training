package com.ducnhu.checkout.kafka;
...
public class ReplyInbox {
    ...
    @KafkaListener(topics = Topics.CART_GET_RESP, groupId = "checkout-service")
    public void onCart(CartGetResponse response) {
        requestReplyClient.complete(response.correlationId(), response);
    }

    @KafkaListener(topics = Topics.SHIP_RATE_RESP, groupId = "checkout-service")
    public void onShip(ShippingRateResponse resp) {
        requestReplyClient.complete(resp.correlationId(), resp);
    }

    @KafkaListener(topics = Topics.CATALOG_PROD_SNAPSHOT_RESP, groupId = "checkout-service")
